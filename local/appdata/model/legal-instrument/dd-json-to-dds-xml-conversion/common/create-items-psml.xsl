                ...
                <document type="stage_1" version="current" level="portable">
                    ...
                    <section id="item" title="Item">
                        <properties-fragment id="Item">
                            <xsl:for-each select="child::*[not(name()='ItemATC' or name()='ItemDispensingRule' or name()='AmtItems' or name()='ItemPrescribingTxtRltd' or name()='ItemRestrictionRltd')]|@*">
                                <property name="{local-name()}" title="{local-name()}" value="{.}" datatype="string"/>
                            </xsl:for-each>
                        </properties-fragment>
                    </section>
                    <section id="amt-items" title="AmtItems">
                        <xsl:for-each select="AmtItems">
                    ...
                    <section id="item-dispensing-rule" title="ItemDispensingRule">
                        <xsl:for-each select="ItemDispensingRule">
                            <xsl:for-each select="child::*">
                                <xsl:variable name="type" select="self::*/local-name()" />
                                <properties-fragment id="Item-Dispensing-Rule-{$type}">
                                    <xsl:for-each select="child::*">
                                        <property name="{local-name()}" title="{local-name()}" value="{.}" datatype="string"/>
                                    </xsl:for-each>
                                </properties-fragment>
                    ...
                    <section id="item-atc" title="ItemATC">
                        <xsl:for-each select="ItemATC/ATCCodeGeneral">
                            <xsl:variable name="atc-general" select="@atc_code" />
                            <xsl:if test="$atc-general!=''">
                                <properties-fragment id="ATC-General">
                                    <property name="atc-general" title="ATC General" value="{$atc-general}" datatype="string"/>
                                </properties-fragment>
                                <xsl:for-each select="ATC">
                                    <xsl:variable name="atc_level" select="@atc_level" />
                                    <properties-fragment id="ATC-{$atc_level}">
                                        <xsl:for-each select="@*">
                                            <property name="{local-name()}" title="{local-name()}" value="{.}" datatype="string"/>
                                        </xsl:for-each>
                                    </properties-fragment>
