...
<xsl:stylesheet
    ...
    <xsl:template match="Schedule">
        ...
        <xsl:variable name="path-document" select="concat($output, $sch-path, 'li/li-schedule-1-part2.psml')" />
        <xsl:variable name="title-li-document" select="'li-schedule-1-part2.psml'" />
        <xsl:result-document href="{$path-document}">
            <document type="li" version="current" level="portable">
                <documentinfo>
                    <uri documenttype="li" title="{$title-li-document}">
                        <displaytitle><xsl:value-of select="$title-li-document"/></displaytitle>
                    </uri>
                </documentinfo>
                <section id="content">
