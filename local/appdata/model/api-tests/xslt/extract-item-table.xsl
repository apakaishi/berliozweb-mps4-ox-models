...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="pbs:program">
        ...
        <xsl:for-each select="pbs:prescribing-rule[pbs:ready-prepared | pbs:infusible]">
            ...
            <xsl:variable name="has-caution"
                select="pbs:prescribing-text-references-list/pbs:caution-reference"/>

            <xsl:variable name="has-restriction"
                select="
            ...
            <xsl:for-each select="*[pbs:product-listing]">
                ...
                <xsl:for-each select="pbs:product-listing">
                    ...
                    <xsl:for-each select="$mpuu ">
                        <xsl:sort select="distinct-values(pbs:code)" />
    ...
    <xsl:template name="MARK_UP_CODE">
        ...
        <xsl:variable name="markup-s90-cp"
            select="
        ...
        <xsl:variable name="markup-s94-private"
            select="
                $prices[pbs:dispensing-rule-reference/pbs:code
                [@rdf:resource = 'http://pbs.gov.au/code/dispensing-rule']
                [ends-with(., 's94-private')]][1]"
            as="element()?"/>

        <xsl:variable name="markup-s94-public"
            select="
                $prices[pbs:dispensing-rule-reference/pbs:code
                [@rdf:resource = 'http://pbs.gov.au/code/dispensing-rule']
                [ends-with(., 's94-public')]][1]"
            as="element()?"/>
    ...
    <xsl:template name="PBS_MPUU_ID">
        ...
        <xsl:text>"</xsl:text>
        <xsl:value-of select="$this-mpuu/pbs:code"/>
        <xsl:text>x",</xsl:text>
    </xsl:template>
