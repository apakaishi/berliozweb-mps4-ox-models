...
<xsl:stylesheet
    ...
    <xsl:key name="items-prescribing-text-relationships" match="items-prescribing-text-relationships/element" composite="yes">
        <xsl:sequence select="pbs_code"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>

    <xsl:key name="prescribing-text" match="prescribing-text/element" composite="yes">
    ...
    <xsl:key name="criteria-parameters-relationships" match="criteria-parameters-relationships/element" composite="yes">
        <xsl:sequence select="criteria_prescribing_txt_id"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>

    <xsl:key name="indications" match="indications/element" composite="yes">
        <xsl:sequence select="indication_prescribing_txt_id"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>

    <xsl:key name="items-restrictions-relationships" match="items-restrictions-relationships/element" composite="yes">
        <xsl:sequence select="pbs_code"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>

    <xsl:key name="restrictions" match="restrictions/element" composite="yes">
        <xsl:sequence select="res_code"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>

    <xsl:key name="restrictions-prescribing-text-relationships" match="restrictions-prescribing-text-relationships/element" composite="yes">
        <xsl:sequence select="res_code"/>
        <xsl:sequence select="schedule_code"/>
    </xsl:key>
